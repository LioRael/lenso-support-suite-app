...
The Resend and Help Center Plugins each provide `lenso.http.endpoint@1` from the
Host's many-valued `web` Slot. The production Host owns ingress attachment and
routing; the App does not author an HTTP binding file.
...
`support.admin/default`, `privacy.export/default`, and
`privacy.retention/default` are deployment placeholders for privileged business,
export, and retention callers. A production Host must either link those exact
Instances or change the typed allowlists to the Instances it actually owns.
...
authenticated and separately permissioned.
...
required by the seven real descriptors but not provided within this suite:
...
Center. Their macro-generated descriptors are obtained from
`NativePluginRegistry::host_catalog`; the six exported descriptor constants are
compared byte-for-structure, while Content Vault is checked for its exact
...
The Web Ingress fixture binds both real HTTP Endpoint providers in deterministic
order.
...
Vault; and Resend plus Help Center, then Support Attachment, before Support Case.
